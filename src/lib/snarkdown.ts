 ...
 * Copied Snarkdown v2.0.0 and converted to TypeScript since the repo is dead.
 */
...
export function parseMD(md: string, prevLinks?: Record<string, string>): string {
	let tokenizer = /((?:^|\n+)(?:\n---+|\* \*(?: \*)+)\n)|(?:^``` *(\w*)\n([\s\S]*?)\n```$)|((?:(?:^|\n+)(?:\t|  {2,}).+)+\n*)|((?:(?:^|\n)([>*+-]|\d+\.)\s+.*)+)|(?:!\[([^\]]*?)\]\(([^)]+?)\))|(\[)|(\](?:\(([^)]+?)\))?)|(?:(?:^|\n+)([^\s].*)\n(-{3,}|={3,})(?:\n+|$))|(?:(?:^|\n+)(#{1,6})\s*(.+)(?:\n+|$))|(?:`([^`].*?)`)|(  \n\n*|\n{2,}|__|\*\*|[_*]|~~)/gm,
		context: string[][] = [],
		out = '',
		links = prevLinks || {},
		last = 0,
		chunk: string, prev: string, token: RegExpExecArray | null, inner: string, t: string;

	function tag(token: string[]): string {
		let desc = TAGS[token[1] || ''];
		let end = context[context.length-1] == token;
		if (!desc) return token[0];
	...
	md = md.replace(/^\[(.+?)\]:\s*(.+)$/gm, (s: string, name: string, url: string) => {
		links[name.toLowerCase()] = url;
		return '';
	}).replace(/^\n+|\n+$/g, '');

	while ( (token = tokenizer.exec(md)) ) {
		prev = md.substring(last, token.index || 0);
		...
		// Headings:
			...
			t = 'h' + (token[14] ? token[14].length : (token[13] && token[13] > '=' ? 1 : 2));
